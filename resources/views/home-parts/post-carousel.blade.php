<div class="fifth_bg">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<div class="carousel_posts1 owl-carousel nav_style2 mb40 mt30">
						<!--CAROUSEL START-->
						<div class="single_post widgets_small post_type5">
							<div class="post_img">
								<div class="img_wrap">
									<a href="#">
										<img src="{{asset('frontend')}}/assets/img/header/slider/hside4.jpg" alt="">
									</a>
								</div>
							</div>
							<div class="single_post_text">
								<h4><a href="post1.html">The home decorations document: photograph of an</a></h4>
								<p>People have been infected</p>
							</div>
						</div>
						<div class="single_post widgets_small post_type5">
							<div class="post_img">
								<div class="img_wrap">
									<a href="#">
										<img src="{{asset('frontend')}}/assets/img/header/slider/hside5.jpg" alt="">
									</a>
								</div>
							</div>
							<div class="single_post_text">
								<h4><a href="post1.html">U.S. Response subash says he will label regions by risk of…</a></h4>
								<p>People have been infected</p>
							</div>
						</div>
						<div class="single_post widgets_small post_type5">
							<div class="post_img">
								<div class="img_wrap">
									<a href="#">
										<img src="{{asset('frontend')}}/assets/img/header/slider/hside6.jpg" alt="">
									</a>
								</div>
							</div>
							<div class="single_post_text">
								<h4><a href="post1.html">Stimul package will transform the government fundamentally.</a></h4>
								<p>People have been infected</p>
							</div>
						</div>
						<div class="single_post widgets_small post_type5">
							<div class="post_img">
								<div class="img_wrap">
									<a href="#">
										<img src="{{asset('frontend')}}/assets/img/header/slider/hside1.jpg" alt="">
									</a>
								</div>
							</div>
							<div class="single_post_text">
								<h4><a href="post1.html">U.S. Response subash says he will label regions by risk of…</a></h4>
								<p>People have been infected </p>
							</div>
						</div>
						<div class="single_post widgets_small post_type5">
							<div class="post_img">
								<div class="img_wrap">
									<a href="#">
										<img src="{{asset('frontend')}}/assets/img/header/slider/hside2.jpg" alt="">
									</a>
								</div>
							</div>
							<div class="single_post_text">
								<h4><a href="post1.html">U.S. Response subash says he will label regions by risk of…</a></h4>
								<p>People have been infected</p>
							</div>
						</div>
						<div class="single_post widgets_small post_type5">
							<div class="post_img">
								<div class="img_wrap">
									<a href="#">
										<img src="{{asset('frontend')}}/assets/img/header/slider/hside3.jpg" alt="">
									</a>
								</div>
							</div>
							<div class="single_post_text">
								<h4><a href="post1.html">U.S. Response subash says he will label regions by risk of…</a></h4>
								<p>People have been infected</p>
							</div>
						</div>
					</div>
					<!--CAROUSEL END-->
				</div>
			</div>
		</div>
	</div>